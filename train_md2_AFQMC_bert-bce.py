...
lcqmc_benchmarkReader= AFQMCBenchmarkReader(data_path="AFQMC")
train_sentence_pairs, train_scores, dev_sentence_pairs, dev_scores = lcqmc_benchmarkReader.get_data()


bert_file_path = "bert-chinese"
print('will load bert is '+ bert_file_path)
external_embed_dim = 768

tokenizer = BertTokenizer.from_pretrained(bert_file_path)
bert_model = BertModel.from_pretrained(bert_file_path).to(device)

sentences_dict = load_data_from_pkl('AFQMC/AFQMC_sentence_bce_dict.pkl')
...
lcqmc_train_loader = DataLoader(lcqmc_train_Dataset, batch_size=32, shuffle=True)
lcqmc_dev_loader = DataLoader(lcqmc_dev_Dataset, batch_size=32, shuffle=False)


print("Model loading...")
myModel = SimilarityModel(bert_model,external_embed_dim).to(device)# 确保模型在GPU上
...
train_model(myModel, lcqmc_train_loader,lcqmc_dev_loader,criterion, optimizer, epochs=15)
myModel = best_model
print('-------------')
